...
def main():
    ...
    model = load_peft_model(model, peft_model_path="jordiclive/falcon-40b-lora-sft-1.1k", tokenizer=tokenizer)
    print('Merge and unload')
    model = model.merge_and_unload()
    model = model.to(torch.bfloat16)
    model.resize_token_embeddings(tokenizer.vocab_size + torch.load(embed_weights).shape[0])
    add_embeddings(model, embed_weights, tokenizer)  # todo needed?
    model.save_pretrained('/mnt/data/jordiclive/falcon/merged_falcon_take2', torch_dtype=torch.bfloat16,
                          max_shard_size="10GB")
    ...
    #     prompts = prompts[local_chunk * local_rank:]
    # else:
    #     prompts = prompts[local_chunk * local_rank: local_chunk * (local_rank + 1)]
    #
    # args_dict = vars(args)
    ...
    #     report_filename = f"{date}_{save_model_name}_sampling_{config_name}_rank_{local_rank}.json"
